...
export default function StatCard({
  ...
  return (
    <div className={`bg-white rounded-xl border border-zinc-200/80 p-4 sm:p-5 shadow-xs transition-shadow hover:shadow-subtle min-w-0 overflow-hidden ${className}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500 truncate" title={title}>
            {title}
          </p>
          <p className="text-xl sm:text-2xl font-bold text-zinc-900 mt-1 tracking-tight truncate" title={typeof value === 'string' ? value : undefined}>
            {value}
          </p>
          {subtitle && (
            <p className="text-xs text-zinc-500 mt-0.5 line-clamp-1" title={subtitle}>
              {subtitle}
            </p>
          )}
        </div>
